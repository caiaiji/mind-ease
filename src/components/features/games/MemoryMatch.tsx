...
const cardStyle = (card: Card): React.CSSProperties => ({
  ...
  ...(card.matched
    ? { background: '#ECFDF5', borderColor: '#A7F3D0', transform: 'scale(0.95)', opacity: 0.7 }
    : card.flipped
      ? { background: 'white', borderColor: '#DDD6FE', boxShadow: '0 4px 15px rgba(0,0,0,0.1)', transform: 'scale(1.05)' }
      : { background: 'linear-gradient(135deg, #DDD6FE, #FECDD3)', borderColor: 'rgba(167,139,250,0.3)', boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }),
})
    ...
    <div style={{ textAlign: 'center' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'center', gap: 8, marginBottom: 24 }}>
        {(Object.keys(DIFFICULTY) as Difficulty[]).map((diff) => (
          <button key={diff} onClick={() => startGame(diff)} style={btnStyle(difficulty === diff)}>
      ...
      <div style={{ display: 'flex', justifyContent: 'center', gap: 24, marginBottom: 24 }}>
        <div style={statBox}>
          <div style={{ fontSize: 12, color: '#9CA3AF' }}>步数</div>
          <div style={{ fontSize: 18, fontWeight: 500, color: '#374151' }}>{moves}</div>
        </div>
        <div style={statBox}>
          <div style={{ fontSize: 12, color: '#9CA3AF' }}>已配对</div>
          <div style={{ fontSize: 18, fontWeight: 500, color: '#A78BFA' }}>{matchedCount} / {totalPairs}</div>
        </div>
        <div style={statBox}>
          <div style={{ fontSize: 12, color: '#9CA3AF' }}>用时</div>
          <div style={{ fontSize: 18, fontWeight: 500, color: '#374151' }}>{formatTime(elapsed)}</div>
        </div>
      </div>
      ...
      <div style={{
        ...
        background: 'rgba(254,225,211,0.3)', justifyContent: 'center', margin: '0 auto', width: 'fit-content',
      }}>
        {cards.map((card) => (
          <button key={card.id} onClick={() => handleCardClick(card.id)} style={cardStyle(card)}>
