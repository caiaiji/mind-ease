...
const GAMES: { id: GameId; title: string; description: string; emoji: string; gradient: string; tag: string }[] = [
  { id: 'bubble-pop', title: '戳泡泡', description: '模拟捏气泡垫，点击就会"啵"地弹开，极其解压', emoji: '🫧', gradient: 'linear-gradient(135deg, rgba(237,233,254,0.5), rgba(237,233,254,0.2))', tag: '解压神器' },
  { id: 'flower-garden', title: '种花花园', description: '点击空地种下种子，等待它们慢慢成长开花', emoji: '🌸', gradient: 'linear-gradient(135deg, rgba(209,250,229,0.5), rgba(209,250,229,0.2))', tag: '治愈养成' },
  { id: 'memory-match', title: '记忆翻牌', description: '翻开卡牌找配对，可爱的 emoji 图案等你发现', emoji: '🃏', gradient: 'linear-gradient(135deg, rgba(254,205,211,0.5), rgba(254,205,211,0.2))', tag: '轻松益智' },
  { id: 'floating-bubbles', title: '漂浮泡泡', description: '点击屏幕生成彩色泡泡，安静的禅意体验', emoji: '✨', gradient: 'linear-gradient(135deg, rgba(219,234,254,0.5), rgba(233,213,255,0.2))', tag: '禅意放松' },
  { id: 'schulte-grid', title: '舒尔特方格', description: '按 1 到 N 的顺序依次点击方格中的数字，训练注意力', emoji: '🧩', gradient: 'linear-gradient(135deg, rgba(254,243,199,0.5), rgba(253,224,71,0.2))', tag: '注意力训练' },
]
    ...
    return (
      <div style={{ padding: '32px 24px 80px', maxWidth: 800, margin: '0 auto' }}>
        <button
          ...
          style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 14, color: '#9CA3AF', background: 'none', border: 'none', cursor: 'pointer', padding: '0 0 24px', marginBottom: 8 }}
        >
          ← 返回游戏列表
        </button>
        ...
        <div style={{ ...glassCard, padding: 24, textAlign: 'center', marginBottom: 32 }}>
          ...
          <h2 style={{ fontFamily: '"ZCOOL XiaoWei", serif', fontSize: 24, color: '#1F2937' }}>{game.title}</h2>
          <p style={{ fontSize: 14, color: '#9CA3AF', marginTop: 4 }}>{game.description}</p>
        </div>
    ...
    <div>
      <div style={{ padding: '48px 24px 0' }}>
        <div style={{ maxWidth: 960, margin: '0 auto' }}>
          <h1 style={{ fontFamily: '"ZCOOL XiaoWei", serif', fontSize: 32, color: '#1F2937', marginBottom: 12 }}>放松小游戏</h1>
        ...
        <div style={{ maxWidth: 960, margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: 24 }}>
          {GAMES.map((game) => (
            <button
              ...
              style={{ ...gameCard, background: game.gradient }}
              ...
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: 16 }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                    <h3 style={{ fontFamily: '"ZCOOL XiaoWei", serif', fontSize: 20, color: '#1F2937' }}>{game.title}</h3>
                    <span style={{
                      padding: '2px 10px', borderRadius: 999, fontSize: 12,
                      background: 'rgba(255,255,255,0.6)', color: '#8B5CF6', fontWeight: 500,
                    }}>{game.tag}</span>
                  </div>
                  <p style={{ fontSize: 14, color: '#6B7280', lineHeight: 1.6 }}>{game.description}</p>
                </div>
              </div>
